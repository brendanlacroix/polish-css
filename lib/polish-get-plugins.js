...
module.exports = function getPlugins(modules, directory) {
  var loadedFromModules   = getPluginModules(modules),
      loadedFromDirectory = getPluginDirectory(directory),
      rules = {};

  if (modules) {
    _.extend(rules, loadedFromModules);
  }

  if (directory) {
    _.extend(rules, loadedFromDirectory);
  }
