...
function processModuleConfigs(moduleConfigs) {
  var plugins = {};
  ...
  moduleConfigs.forEach(function(config) {
    ...
    if (!_.includes([0, 1, 2], config.severity)) {
...
function getConfigFromCWD() {
  var plugins    = {},
      configPath = path.join(process.cwd(), '.polish.json');
...
module.exports = function loadPlugins(moduleConfigs) {
  ...
  if (!moduleConfigs) {
    moduleConfigs = getConfigFromCWD();
  }

  plugins = processModuleConfigs(moduleConfigs);
